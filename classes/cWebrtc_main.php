 ...
 * webrtc_main Class
 *
 * @used in  				webrtc_main Function
 * @created  				Hyella Nathan | 06:27 | 20-Nov-2024
 * @database table name   	webrtc_main
 */

	class cWebrtc_main extends cCustomer_call_log{
		public $class_settings = array();

		private $current_record_id = '';
		
		public $customer_source = '';
		public $table_name = 'webrtc_main';
		
		public $default_reference = '';
		
		public $label = 'Realtime Communication Main';
		...
		function __construct(){
			...
			if( file_exists( dirname( __FILE__ ).'/dependencies/webrtc_main.json' ) ){
				$return = json_decode( file_get_contents( dirname( __FILE__ ).'/dependencies/webrtc_main.json' ), true );
		...
		function webrtc_main(){
			...
			break;
			case 'display_app_chat_screen':
			case 'display_app_chat_screen2':
				$returned_value = $this->_display_app_chat_screen();
			break;
			}
		...
		protected function _display_app_chat_screen(){
			$js = [];
			$data = [];
			...
			if ((isset($this->class_settings['html_replacement_selector']) && $this->class_settings['html_replacement_selector'])) {
				...
				$data['params'] = '&html_replacement_selector=' . $this->class_settings['html_replacement_selector'];

				$data['html_replacement_selector'] = $this->class_settings['html_replacement_selector'];
			}
			
			$data[ 'current_user' ] = isset( $_SESSION[ 'reuse_settings' ][ 'user_cert' ] ) ? $_SESSION[ 'reuse_settings' ][ 'user_cert' ] : '';
			$data[ 'current_user' ][ 'username' ] = $data[ 'current_user' ][ 'initials' ];
